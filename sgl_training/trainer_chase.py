...
class Trainer():
    ...
    def train(self):
        ...
        for batch, (data_pack, _) in enumerate(self.loader_train):
            ...
            timer_data.hold()
            timer_model.tic()
            hr, ve, te, pm, _,_ = data_pack
            self.optimizer.zero_grad()
            enh, estimation= self.model(hr, 1)
            #loss = self.bce_loss(estimation, ve)  #l1 supervisions
            #loss = self.ploss(medmap, med) + 
            loss = self.loss(estimation, ve, pm) + self.loss(estimation, te, pm)# + 0.01*self.tv_loss(enh)# + self.pbce_loss(estimation, ve, pm*ma)  #l1 supervisions
            loss.backward()
        ...
        for idx_data, d in enumerate(self.loader_test):
            for idx_scale, scale in enumerate(self.scale):
                ...
                for data_pack, filename in tqdm(d, ncols=80):
                    data_pack = self.prepare(data_pack)
                    hr, ve, te, pm, ve_thin, ve_thick = data_pack
                    ...
                    ve = ve * 255.
                    ve_thin = ve_thin * 255.
                    ve_thick = ve_thick * 255.
                    est[est>100] = 255
                    est[est<=100] = 0
                    est = est[:, :, 0:960, 0:999]
                    dmap = dmap[:, :, 0:960, 0:999]
                    est_o = est_o[:, :, 0:960, 0:999]
                    enhance = enhance[:, :, 0:960, 0:999]
                    ve = ve[:, :, 0:960, 0:999]
                    hr = hr[:, :, 0:960, 0:999]
