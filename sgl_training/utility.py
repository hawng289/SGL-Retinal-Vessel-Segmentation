...
class checkpoint():
    ...
    def save_results(self, dataset, filename, save_list, scale):
        if self.args.save_results:
            ...
            postfix = ('Enhance','Vessel', 'Prob', 'Raw', 'PenaltyMapHard')
...
def visualize(est, label, npf=False):
    ...
    est_color[np.logical_and(est_in == 255 , label == 0), :] = [0, 255, 0]  #red: false positive
    return torch.tensor(np.expand_dims(np.transpose(est_color, (2,0,1)), 0))

def visualize_dmap(est, label):
    BCE_loss = F.binary_cross_entropy(est, label, reduce=False)
    pt = torch.exp(-BCE_loss)
    dmap = (1-pt)**2
    return dmap

def calc_boundiou(est, label, pmap):
