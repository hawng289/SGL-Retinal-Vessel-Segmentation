        ...
        for idx_data, d in enumerate(self.loader_test):
            for idx_scale, scale in enumerate(self.scale):
                ...
                for data_pack, filename in tqdm(d, ncols=80):
                    data_pack = self.prepare(data_pack)
                    hr, ve, pm, ve_thin, ve_thick = data_pack
                    ...
                    ve = ve[:, :, 0:960, 0:999]
                    ve_thin = ve_thin[:, :, 0:960, 0:999]
                    ve_thick = ve_thick[:, :, 0:960, 0:999]
                    hr = hr[:, :, 0:960, 0:999]
                    est = utility.quantize(est, self.args.rgb_range)
                    estnp = np.transpose(est[0].cpu().numpy(), (1,2,0)) / 255.
                    est_thin, est_thick = data.common_chase.ttsep(estnp)
